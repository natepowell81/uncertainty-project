...
def run_training_C(population, num_timesteps, training_list):
    for individual in population:
        ...
        for position in training_list:
            ...
            index = index + 1
        individual.fit = (1.0 / (1.0 + (sum(cat_list[0][:])/7.0)))
    return population

def run_training_second_C(population, POP_SIZE, num_timesteps, training_list):
    move_list = []
    new_individuals = Fill_C(population, POP_SIZE)
...
def run_testing_C(test_list, population, timestep):
    timestep_list = []
    ...
    newpath = r'/Users/np/Desktop/Bullet/bullet_make/Demos/RagdollDemo/Debug/Images/C/Individuals_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d')
    if not os.path.exists(newpath):
        os.makedirs(newpath)
        
    newpath = r'/Users/np/Desktop/Bullet/bullet_make/Demos/RagdollDemo/Debug/Images/C/Individuals_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d')+'/Individuals'+str(individual.uid)+'_data_for_neurons'
    if not os.path.exists(newpath):
        os.makedirs(newpath)
    ...
    for position in test_list:
        ...
        Send_Values_ToFile(values_save, 'Images/C/Individuals_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d')+'/Individuals'+str(individual.uid)+'_data_for_neurons/'+str(index)+str(individual.uid)+position[6]+position[7]+'Categorizeandmovement.txt')
    ...
    Send_Values_ToFile(parent, 'Images/C/Individuals_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d')+'/Individuals'+str(individual.uid)+'_data_for_neurons/'+str(index)+str(individual.uid)+position[6]+position[7]+'_ANN.txt')
...
def AGE_individuals_C(population):
    for individual in population:
        individual = individual.age_()
    return population
        
def Inject_C(population):
    population.append(Individual())
    return population        
    
def Fill_C(population, POP_SIZE):
    new_individuals = []
    ...
    new_individuals = Inject_C(new_individuals)
    return new_individuals

def del_doms_C(population):
...
def Determine_Dom_C(population):
...
def Determine2_Dom_C(population):
...
def dominate_C(population):
    print "fit", [population[i].fit for i in range(0, len(population))][:10] 
    population = Determine2_Dom_C(population)
    print "dom", [population[i].bool for i in range(0, len(population))]
    
    population = sort_by_dom_C(population)
    population = sort_by_fit_C(population)
    #print len(population)
    population = del_doms_C(population)
    #print len(population)
...
def initial_C(POP_SIZE, num_timesteps, training_list):
    oldpopulation = [Individual() for i in range(0, POP_SIZE)]
    oldpopulation = run_training_C(oldpopulation, num_timesteps, training_list)
    newpopulation = dominate_C(oldpopulation)
    return newpopulation
    
def secondary_C(newpopulation, POP_SIZE, num_timesteps, training_list):
    newpopulation = run_training_second_C(newpopulation, POP_SIZE, num_timesteps, training_list)
    newpopulation = dominate_C(newpopulation)
    return newpopulation
    
def iterations_C(generations, POP_SIZE, num_timesteps):
    Send_Values_ToFile_two([num_timesteps], 'timesteps.txt')
    training_list, test_list = positions()
    for i in range(0, generations):
        print "Gen =", i
        if i == 0:
            newpopulation = initial_C(POP_SIZE, num_timesteps, training_list)
            newpopulation = AGE_individuals_C(newpopulation)
            newpopulation2 = newpopulation  
        if i > 0:
            newpopulation = secondary_C(newpopulation, POP_SIZE, num_timesteps, training_list)
            newpopulation = AGE_individuals_C(newpopulation)  
    
    run_testing_C(test_list, newpopulation, num_timesteps) 
    #plt.show()
...
if __name__ == "__main__":
    population_dict = {}
    for i in range(0,1):
        newpop = iterations_C(10, POP_SIZE, num_timesteps)
